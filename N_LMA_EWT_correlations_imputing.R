...
df <- read.csv("data_gapfilling_pigments_imputed.csv", sep = ",")

colSums(!is.na(df))


# Rename columns to be filled up for code structure below
colnames(df)[colnames(df) == "LMA_gm2"] <- "r"
colnames(df)[colnames(df) == "EWT_mgcm2"] <- "m"
...
#df <- as.data.frame(dt)
#colnames(df)[colnames(df) == "LMA_gm2"] <- "r"
#colnames(df)[colnames(df) == "nitrogenc_mgg"] <- "m"
...
dt[, c("LMA_gm2", "EWT_mgcm2") := .(r.comp, m.comp)]
...
write.csv(df_fin, "data_gapfilling_LMA_EWT_imputed.csv", row.names = FALSE)
